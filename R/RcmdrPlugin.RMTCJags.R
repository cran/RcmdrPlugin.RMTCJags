...
RunMTCModel <- function(){
	...
	modelValue <- trim.blanks(tclvalue(modelName))
		if (!is.valid.name(modelValue)) {
			UpdateModelNumber(-1)
			errorCondition(recall = RunMTCModel, message = sprintf(gettextRcmdr("\"%s\" is not a valid name."), 
							modelValue))
			return()
		}
	...
	if(models=="fem")
		{
		Teste_fem <- paste("Model <- 'model{", "for(i in 1:N){", "logit(p[s[i],t[i]]) <- mu[s[i]] + d[t[i]] - d[b[i]]",
	...
	if(models=="rem")
		{
		Teste_rem <- paste("Model <- 'model{", "for(i in 1:N){", "logit(p[i])<-mu[s[i]]+delta[i]  * (1-equals(t[i],b[i]))",
	...
	if(models=="threerem")
		{
		Teste_threerem <- paste("Model <- 'model{","sw[1] <- 0","for(i in 1:N)  {","logit(p[i])<-mu[s[i]]+ delta[i]  * (1-equals(t[i],b[i]))",
	...
	if(models=="marem")
		...
		Teste_marem <- paste("Model <- 'model{","for(i in 1:NS){","w[i,1] <-0","delta[i,t[i,1]]<-0","mu[i] ~ dnorm(0,.0001)","for (k in 1:na[i])  {",
...
ForestPlot <- function(){
	...
	command <- paste('Teste_ForestPlot <- summary(',modelValue,'_aux2)', sep="")
        doItAndPrint(command)
	justDoIt(paste('lod <- Teste_ForestPlot$statistics[,1]', sep=""))
	justDoIt(paste('sd_lod <- Teste_ForestPlot$statistics[,2]', sep=""))
	justDoIt(paste('N_Trat <- length(Teste_ForestPlot$statistics[,1])', sep=""))
...
GelmanRubinPlot <- function(){
	...
	command <- paste('Teste_GelmanRubin <- summary(',modelValue,'_aux)', sep="")
        justDoIt(command)
	justDoIt(paste('N_Trat <- length(Teste_GelmanRubin$statistics[,1])', sep=""))
